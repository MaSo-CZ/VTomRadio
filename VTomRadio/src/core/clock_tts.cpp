...
static bool clock_tts_is_quiet_time(const tm* tm_struct) {
    if (!clock_tts_quiet_hours_enabled || !tm_struct) { return false; }
    uint16_t nowMin = static_cast<uint16_t>(tm_struct->tm_hour * 60 + tm_struct->tm_min);
    if (clock_tts_quiet_from_minutes == clock_tts_quiet_to_minutes) { return false; }
    if (clock_tts_quiet_from_minutes < clock_tts_quiet_to_minutes) { return nowMin >= clock_tts_quiet_from_minutes && nowMin < clock_tts_quiet_to_minutes; }
...
static bool clock_tts_is_supported_lang(const char* lang) {
    ...
    return strncmp(lang, "EN", 2) == 0 || strncmp(lang, "HU", 2) == 0 || strncmp(lang, "PL", 2) == 0 || strncmp(lang, "NL", 2) == 0 || strncmp(lang, "DE", 2) == 0 || strncmp(lang, "RU", 2) == 0 ||
           strncmp(lang, "RO", 2) == 0 || strncmp(lang, "FR", 2) == 0 || strncmp(lang, "GR", 2) == 0;
}
...
void clock_tts_enable(bool enable) {
    ...
    if (!enable) {
        if (clock_tts_prev_volume > 0) { player.setVolume(clock_tts_prev_volume); }
...
void clock_tts_setup() {
    ...
    clock_tts_fade_volume = -1;
    clock_lastTTSMillis = millis(); // Beállítjuk a boot időpontját
    clock_ttsActive = false;
    clock_tts_started_at = 0;
    clock_tts_last_progress_at = 0;
    clock_tts_last_audio_time = 0;
    clock_tts_audio_progress_seen = false;
    clock_tts_resume_after = false;
    // --- BOOT GUARD 1: Idő inicializálása az indítási vak-trigger ellen --- (Automatikus lejátszás indítása hiba #11)
    tm localTm = clock_tts_get_display_time_snapshot();
    if (localTm.tm_year + 1900 >= 2020) {
        clock_lastMinute = localTm.tm_min; // Lementjük a jelenlegi percet
    } else {
        clock_lastMinute = -1;
    }
    ...
    if (!clock_tts_is_supported_lang(clock_tts_language)) { clock_tts_set_language("HU"); }
}
...
void clock_tts_loop() {
    ...
    // --- BOOT GUARD 2: Az első 6 másodpercben letiltjuk a TTS-t --- (Automatikus lejátszás indítása hiba #11)
    ...
    if (clock_tts_fading_up) {
        if (clock_tts_fade_volume == -1) { clock_tts_fade_volume = 0; }
    ...
    if (tm_struct->tm_year + 1900 < 2020) { return; }
    ...
    if (clock_ttsActive) {
        ...
        if (!player.isRunning() || (nowMillis - clock_tts_started_at > CLOCK_TTS_MAX_ACTIVE_MS)) { shouldRecover = true; }
        if (!clock_tts_audio_progress_seen && (nowMillis - clock_tts_started_at > CLOCK_TTS_NO_PROGRESS_MS)) { shouldRecover = true; }
